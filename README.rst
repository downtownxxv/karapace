...
.. list-table::
   ...
   * - ``iterations``
     - Optional. PBKDF2 iteration count the ``password_hash`` was generated with (ignored for ``scrypt``). When omitted, a legacy default of 5000 is assumed so authorization files created before this field existed keep working. New hashes generated by ``karapace_mkpasswd`` include it, so the exact work factor round-trips to verification.

Password hashing can be done using ``karapace_mkpasswd`` tool, if installed, or by invoking directly with ``python -m karapace.core.auth``. The tool generates JSON entry with these fields. The PBKDF2 work factor defaults to a strong value and can be tuned with ``-i/--iterations``. Note that under HTTP Basic authentication the hash is recomputed on every authenticated request, so a higher iteration count increases per-request CPU cost. ::
...
.. list-table::
   ...
   * - ``resource``
     - A regular expression matched against the *whole* accessed resource string (the match is fully anchored). Use an explicit wildcard such as ``.*`` to authorize a family of resources: a literal like ``Subject:orders`` matches only that exact resource, while ``Subject:orders.*`` also matches ``Subject:orders-secret``.
...
.. list-table::
   ...
   * - ``Subject:<subject_name>``
     - Controls authorization to subject. The whole ``Subject:<subject_name>`` string is matched against the (fully anchored) ``resource`` pattern, so ``<subject_name>`` must be written to match the accessed subject exactly or via an explicit wildcard (e.g. ``Subject:general.*``).
